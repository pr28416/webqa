...
- **Tech Stack:**
  ...
  - Stagehand (@browserbasehq/stagehand ^3.0.5) for AI-powered browser automation
  - Drizzle ORM (^0.45.0) + postgres (^3.4.7) for database access
  - Supabase (PostgreSQL) for database and backend services
...
- **File Structure:**
  ...
  - `lib/db/` – Database layer (Drizzle ORM setup, schema definitions)
  - `lib/db/schema/` – Drizzle schema definitions
  - `lib/agents/` – AI agent implementations
  - `lib/stagehand/` – Stagehand instance management
  - `lib/tools/` – Tool definitions for AI agents
...
- **Key Concepts:**
  ...
  - ChatView component provides AI-powered chat interface with Stagehand integration
  - Database layer uses Drizzle ORM with Supabase PostgreSQL
  - Migrations are managed via Supabase CLI (generated by Drizzle Kit)
...
- **Database:**
  ...
  - `pnpm db:migrate` (apply migrations to database via Drizzle Kit)
  - `pnpm db:push` (push schema changes directly - dev only)
  - `pnpm db:studio` (open Drizzle Studio for database management)
  - `pnpm db:check` (check for schema conflicts and issues)
  - `supabase db push` (apply migrations via Supabase CLI - recommended)
  - `supabase status` (view database connection info)
...
// ✅ Good - Insert with proper types
async function createBrowserSession(data: {
  sessionId: string;
  cdpWsUrl: string;
}) {
  const [session] = await db
...
// ❌ Bad - Raw SQL strings (use Drizzle's query builder)
const sessions = await db.execute(
  `SELECT * FROM browser_sessions WHERE session_id = '${sessionId}'`
);

// ❌ Bad - No type safety
const data = await db.select().from(schema.browserSessions);
// data is 'any' - should explicitly type or use schema inference
```
...
5. **Apply**: `supabase db push` (or `pnpm db:migrate`)
6. **Use in code**: Import from `@/lib/db`
...
- ✅ **Always:**
  ...
  - Use environment variables for API keys (`ONKERNEL_API_KEY`, `DATABASE_URL`)
  - Validate search params and request bodies (they can be `null`)
  - **Use Drizzle ORM for database access** - import from `@/lib/db`
  - **Define database schemas in `lib/db/schema/`** - use TypeScript with Drizzle's schema builder
  - **Use Drizzle's query builder** - avoid raw SQL strings
  - **Generate migrations with `pnpm db:generate`** before applying schema changes
  - **Apply migrations via Supabase CLI** (`supabase db push`) for consistency
...
- ⚠️ **Ask first:**
  ...
  - Major database schema changes (new tables, significant refactoring)
  - Changing database migration strategy or tooling
...
- 🚫 **Never:**
  ...
  - Use raw SQL strings (use Drizzle's query builder instead)
  - Manually edit migration files in `supabase/migrations/` (generate via Drizzle)
  - Bypass database schema validation (always use Drizzle schema types)
  - Apply migrations without reviewing the generated SQL first
